/**
 * SHADOWED FROM VOLTO 19.3.0.
 *
 * This file is a verbatim copy of
 * `@plone/volto/components/manage/Toolbar/Toolbar.jsx` with the changes
 * listed below and nothing else. Keep it that way: when Volto is upgraded,
 * diff the upstream file against this one, take upstream's version wholesale
 * and re-apply only these changes.
 *
 * WHY IT IS SHADOWED. Volto has no extension point for the avatar. The only
 * pluggable in `PersonalTools` is `toolbar-user-menu`, at the end of the menu
 * *list*; `toolbar-personal` is a DOM id on a button in `Toolbar`, not a
 * pluggable. Neither the toolbar button's icon nor the menu's avatar block
 * can be replaced without replacing the component around it.
 *
 * WHAT CHANGED:
 * - the `#toolbar-personal` button renders `<UserAvatar>` instead of
 *   the generic `user.svg` icon, so the button shows *who* is signed
 *   in rather than that somebody is;
 * - the now-unused `userSVG` import is replaced by that component's.
 */
/**
 * Toolbar component.
...
import UserAvatar from '@plone-collective/volto-identity/components/UserMenu/UserAvatar';
