...
- Pointed the frontend settings reference at `types/settings`, where `IdentitySettings` is now declared. @ericof [#98](https://github.com/collective/pas-plugins-identity/issues/98)
...
- Documented `@portrait/<userid>` as an anonymous endpoint in the endpoints reference, and corrected its note on registrations: only the authenticated services are registered with `zope2.View`, while the anonymous ones are `zope.Public`. @ericof [#68](https://github.com/collective/pas-plugins-identity/issues/68)
- Documented `api.portrait.store` and `api.portrait.sync_portrait` in the Python API reference, including that `sync_portrait` reports a refusal only in the log. @ericof [#78](https://github.com/collective/pas-plugins-identity/issues/78)
- Stated in the security guarantees and the threat model that a GenericSetup import is held to the same provider icon rule as the control panel: a document that is not an SVG is refused, and an SVG is stored sanitized. @ericof [#79](https://github.com/collective/pas-plugins-identity/issues/79)
- Documented the GitHub provider's address preference, and how linking by email handles several verified addresses, in the GitHub, linking and troubleshooting guides and in the provider form, settings and shipped drivers references. Documented confirming an address at first sign-in in the settings, profiles and groups, endpoints and frontend references, and brought the upgrade guide's list of steps up to date with 1005, 1006 and 1007. @ericof [#80](https://github.com/collective/pas-plugins-identity/issues/80)
- Listed the providers control panel's settings, add and edit routes in the frontend reference. @ericof [#81](https://github.com/collective/pas-plugins-identity/issues/81)
- Documented which identity settings are required, moved the four derived user and group records out of the settings form's tabs, and corrected the default profile's version to 1006. @ericof [#82](https://github.com/collective/pas-plugins-identity/issues/82)
- Documented exporting providers from the control panel, the `@identity-providers/@export` endpoint, and the permission both provider exports need. @ericof [#83](https://github.com/collective/pas-plugins-identity/issues/83)
- Documented ordering the login buttons by dragging the provider list, the `PATCH @identity-providers` endpoint behind it, and the Order field leaving the provider form. @ericof [#84](https://github.com/collective/pas-plugins-identity/issues/84)
- Documented the Principals folder content type, its add permission, and the error raised when a container's parent refuses its type. @ericof [#85](https://github.com/collective/pas-plugins-identity/issues/85)
- Documented the default icon each shipped driver draws its login button with, where each icon comes from, and how a provider's own upload replaces it. @ericof [#86](https://github.com/collective/pas-plugins-identity/issues/86)
- Rewrote the Keycloak recipe around the new `keycloak` driver, with its claims re-read from a Keycloak 26.0 token, and added the driver to the shipped drivers and provider form references. @ericof [#87](https://github.com/collective/pas-plugins-identity/issues/87)
- Pointed the frontend README's Storybook section at `src/stories/`, where the `withPage` and `withUserMenu` decorators now live. @ericof [#88](https://github.com/collective/pas-plugins-identity/issues/88)
- Documented `config.settings.identity`, the one place the frontend add-on's settings live, including that a configured avatar palette is not checked for contrast. @ericof [#89](https://github.com/collective/pas-plugins-identity/issues/89)
- Documented the Sign-in block: how a project offers it, what it shows, and where each line comes from. @ericof [#99](https://github.com/collective/pas-plugins-identity/issues/99)
- Documented how to turn off the login page's redirect to a site's only provider, and when it does not redirect. @ericof [#100](https://github.com/collective/pas-plugins-identity/issues/100)
