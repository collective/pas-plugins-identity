...
| Property | Detail |
|---|---|
| A secret is write-only through the API and the control panel | Both serialize a stored secret as a mask that round-trips, so editing a provider never requires retyping its credential and never sends the value out. The audit log records no credential or token. **A GenericSetup export is not covered**: it carries the stored value, as does the fragment `@identity-providers/<id>/export` returns, and both need `Manage portal`. See {doc}`/concepts/secrets`. |
