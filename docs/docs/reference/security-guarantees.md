...
| Property | Detail |
|---|---|
| A secret is write-only through the API and the control panel | Both serialize a stored secret as a mask that round-trips, so editing a provider never requires retyping its credential and never sends the value out. The audit log records no credential or token. **An export is not covered**: a GenericSetup export carries the stored value, and so do the documents `@identity-providers/@export` and `@identity-providers/<id>/export` return. A GenericSetup export needs `Manage portal`; the two API exports need `pas.plugins.identity: Export Identity Providers`, which a default install grants to Manager alone. See {doc}`/concepts/secrets`. |
