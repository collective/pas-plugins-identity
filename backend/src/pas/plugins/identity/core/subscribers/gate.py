...
def incomplete_profile_url(userid: str) -> str | None:
    ...
    The same question the gate asks on every page load, asked from outside a
    request -- the authorization endpoint needs it before it will hand out a
    code, because a relying party must not be told about a user whose account
    this site is still waiting on.

    Answered from a catalog brain and from the same registry record the gate
    reads, so asking it on every authorization request costs no object load
    and cannot disagree with the rest of the flow.

    Honours the enforcement switch. A site that has turned the gate off has
    said that an incomplete profile is a suggestion, and an authorization
    endpoint refusing to proceed would not be a suggestion.

    :param userid: Canonical Plone userid.
    :returns: The absolute URL of the edit form, or ``None`` when the profile
        is complete, absent, or nothing is being enforced.
    """
    if not enforcing():
        return None
    brain = _incomplete_profile(userid)
    return f"{brain.getURL()}/edit" if brain is not None else None
