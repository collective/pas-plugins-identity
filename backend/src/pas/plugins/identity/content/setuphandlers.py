...
:mod:`pas.plugins.identity.content.catalog`.
...
from pas.plugins.identity.content.pas import PLUGIN_TITLE
from pas.plugins.identity.content.principals import clear_core_records
from pas.plugins.identity.content.principals import sync_core_records
...
def post_install(context: SetupTool) -> None:
    ...
    work, and :func:`~pas.plugins.identity.content.subscribers.ensure_profile`
...
def rebuild_catalog(context: SetupTool) -> None:
    ...
    findings by :func:`~pas.plugins.identity.content.doctor.check` needs to be
...
def post_uninstall(context: SetupTool) -> None:
    ...
    clear_core_records()
    logger.info("Uninstalled the content layer; Profile content was left in place.")
