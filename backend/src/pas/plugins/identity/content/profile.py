...
* :class:`~pas.plugins.identity.content.deserializer.UseridIsPermanent`
  refuses a change over the REST API with a 400 rather than a traceback;
* :mod:`pas.plugins.identity.content.doctor` treats a duplicate as an error
...
:data:`~pas.plugins.identity.content.pas.PROPERTY_FIELDS`: those are served
...
class IProfileSchema(model.Schema, IUserContent):
    ...
    :func:`~pas.plugins.identity.content.subscribers._profile_id` has always
