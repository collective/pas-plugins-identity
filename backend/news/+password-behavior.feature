...
Core does the authenticating, through a new `ICredentialStorage` contract the behavior provides. The `[profile]` layer serves properties, enumeration and groups and never becomes a way to log in — the plugin that authenticates a userid is the one `@users` reports as its source, and an optional property store must not change a site's answer to where an account came from.
