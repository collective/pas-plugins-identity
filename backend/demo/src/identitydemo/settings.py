...
#: from the issuer because in the manual stack Volto and the backend answer on
#: the same host; a deployment where they do not would configure it separately.
#:
#: Read with ``os.environ.get`` rather than through :func:`_url`, because the
#: empty string is a *meaningful* value here and that helper cannot express
#: it: empty means "render the standalone page". The federation test stack
#: sets it empty, and has to -- it runs the two backends and no frontend, so
#: pointing the consent screen at a frontend route sent every first
#: authorization to a 404 and the flow could never be completed.
IDP_CONSENT_URL = os.environ.get(
    "DEMO_IDP_CONSENT_URL", f"{IDP_PUBLIC_URL}/oauth-consent"
).rstrip("/")
