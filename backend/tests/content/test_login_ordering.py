...
:mod:`tests.content.test_first_login` fires the events instead of logging in,
...
Core makes two writes at the end of a login -- the provider's mapped claims,
...
plugin that claims the property sheet, and ``portraits.store`` asks for the
``IProfileSupport`` utility.
...
class TestTheFirstLoginWritesToTheClaimedStore:
    ...
    def test_the_mapped_claim_lands_on_the_profile(self):
        """Which it did before the fix too -- the content layer applies the
