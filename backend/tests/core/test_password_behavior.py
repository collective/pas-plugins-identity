...
def enabled(portal):
    ...
    The schema cache is invalidated explicitly. Dexterity caches a type's
    behavior registrations per FTI, so writing ``behaviors`` and stopping
    leaves an object built earlier in the same run adapting against the old
    list -- and the symptom is ``ICredentialStorage`` answering ``None`` for a
    type that now carries the behavior. Setting it through the types control
    panel goes through the machinery that invalidates; a fixture assigning the
    attribute has to do it by hand.
    ...
    SCHEMA_CACHE.invalidate(PROFILE_PORTAL_TYPE)
    return portal
