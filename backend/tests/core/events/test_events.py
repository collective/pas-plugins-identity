...
class TestExternalIdentityAuthenticated:
    ...
    def _setup(self, authenticated) -> None:
        self.event = authenticated

    def test_carries_the_identity(self):
        """Provider and subject together name the external identity."""
        assert self.event.userid == "userid-1"
        assert self.event.provider == "dex"
        assert self.event.subject == "CgVlcmljbxIFbG9jYWw"

    def test_carries_claims(self):
        """Claims travel with the event, unmodified."""
        assert self.event.claims == CLAIMS

    def test_flags_a_first_login(self):
        """A minted userid and a fresh identity are reported separately."""
        assert self.event.is_new_user is True
        assert self.event.is_new_identity is True

    def test_repeat_login_is_neither_new(self):
        """The same human logging in again is new in no sense."""
...
class TestUserClaimsRefreshed:
    def test_carries_the_fresh_claims(self):
        """A refresh hands subscribers the new claims, not a diff."""
